...
void load_elf(const char *path) {
    ...
    char buf[sizeof(ELF_HEADER_t)];
    memset(buf, 0, sizeof(ELF_HEADER_t));
    int result = vfs_read(fd, buf, sizeof(ELF_HEADER_t));
    if(result == -1) {
        printf("Failed to read ELF file\n");
        return;
    }

    ELF_HEADER_t *elf_header = (ELF_HEADER_t *)buf;
    ...
    for(size_t i = 0; i < elf_header->program_header_entry_count; i++) {
        ...
        printf("HEADER NO %d: type %d, flags %d, vaddr %x, filesz %d, memsz %d\n", 
               ...
               (uint32_t)program_header.file_size, (uint32_t)program_header.memory_size);
