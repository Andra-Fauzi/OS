...
void setup_ehci() {
    ...
    operation_regs->ctrl_ds_segment = 0; // Ensure 4GB segment limit (32-bit addressing)

    operation_regs->config_flag = 1; // enable configuration flag
    
    // Start controller with ONLY async schedule first (for control transfers)
    operation_regs->usb_command = (1 << 5) | (1 << 0); // Async Schedule Enable + Run
    
    // Wait for controller to start running
    timeout = 0x1FFFFFFF;
    while(operation_regs->usb_status & (1 << 12)) { // Wait until NOT halted
        timeout--;
        if(timeout <= 0) {
            printf("Error: EHCI failed to start\n");
            return;
        }
    }
    
    // Wait for async schedule to be ready
    timeout = 0x1FFFFFFF;
    while(!(operation_regs->usb_status & (1 << 15))) { // Async Schedule Status
        timeout--;
        if(timeout <= 0) {
            printf("Error: Async schedule not ready\n");
            return;
        }
    }
    
    printf("EHCI Controller started successfully\n");
    printf("Status after start: CMD=%x STS=%x\n", operation_regs->usb_command, operation_regs->usb_status);
}
...
void setup_mouse_ehci() {
    ...
    while (processed < total_len) {
        ...
        if (type == 5) { // Endpoint Descriptor
            ...
            if (ep->endpoint_address & 0x80) { // Direction IN
                endpoint = ep->endpoint_address & 0x0F;
                max_packet_size = ep->max_packet_size & 0x7FF;
                printf("Found IN Endpoint: %d, Max Packet Size: %d\n", 
                       endpoint, max_packet_size);
                break;
            } else {
                printf("Found OUT Endpoint: %d (skipping)\n", ep->endpoint_address & 0x0F);
            }
        }
        ptr += len;
        processed += len;
...
void input_mouse_ehci() {
    ...
    // Diagnostics
    // printf("EHCI Stat: CMD=%x STS=%x FR=%d\n", operation_regs->usb_command, operation_regs->usb_status, operation_regs->frame_index);

    // Check if controller is halted - don't proceed if so
    if(operation_regs->usb_status & (1 << 12)) {
        printf("ERROR: Controller is HALTED! Cannot proceed.\n");
        return;
    }

    if (mouse_qh == NULL) {
        printf("Initializing mouse QH...\n");
        ...
        // Check alignment
        printf("QH virt=%x phys=%x (align=%s)\n", mouse_qh, (uint32_t)VIRT_TO_PHYS(mouse_qh), 
               (((uint32_t)VIRT_TO_PHYS(mouse_qh) & 0x1F) == 0) ? "OK" : "BAD");
        printf("qTD virt=%x phys=%x (align=%s)\n", mouse_qtd, (uint32_t)VIRT_TO_PHYS(mouse_qtd),
               (((uint32_t)VIRT_TO_PHYS(mouse_qtd) & 0x1F) == 0) ? "OK" : "BAD");
        printf("Buf virt=%x phys=%x\n", mouse_data_buf, (uint32_t)VIRT_TO_PHYS(mouse_data_buf));
        
        memset((void*)mouse_qh, 0, sizeof(ehci_queue_head_t));
        memset((void*)mouse_qtd, 0, sizeof(ehci_qtd_t));
        memset((void*)mouse_data_buf, 0, max_packet_size);
        ...
        // FIXED: Multiplier = 0 (bits 30-31, meaning 1 transaction per microframe)
        // S-mask = 0xFE (execute in microframes 1-7), C-mask = 0x00, RL=3 (NAK reload)
        mouse_qh->endpoint_capabilities = (3 << 28) | (0 << 30) | (0x01 << 0); 
        
        // FIXED: Initialize QH overlay to ensure clean state
        mouse_qh->current_qtd_address = 0;
        mouse_qh->next_qtd = 1;  // Will be set properly below
        mouse_qh->alt_next_qtd = 1;
        mouse_qh->token = 0;  // Clear overlay token
        
        // Token: DATA0 initially, Total Bytes, CERR=3, PID=IN, Active
        ...
        // NOW safe to enable periodic schedule (frame list populated!)
        printf("Enabling periodic schedule...\n");
        operation_regs->usb_command |= (1 << 4);
        
        volatile uint32_t timeout = 0x1FFFFFFF;
        while(!(operation_regs->usb_status & (1 << 14))) {
            timeout--;
            if(timeout <= 0) {
                printf("Error: Periodic schedule timeout\n");
                return;
            }
        }
        printf("Periodic enabled successfully\n");
        
        printf("Mouse periodic QH initialized at endpoint %d\n", endpoint);
        printf("QH phys=%x, qTD phys=%x, buf phys=%x\n", 
               (uint32_t)VIRT_TO_PHYS(mouse_qh),
               (uint32_t)VIRT_TO_PHYS(mouse_qtd),
               (uint32_t)VIRT_TO_PHYS(mouse_data_buf));
        printf("QH.horiz=%x QH.endp_char=%x QH.endp_cap=%x\n",
               mouse_qh->horizontal_link_pointer,
               mouse_qh->endpoint_characteristics,
               mouse_qh->endpoint_capabilities);
        printf("QH.next_qtd=%x QH.token=%x\n", mouse_qh->next_qtd, mouse_qh->token);
        printf("qTD.token=%x qTD.buf[0]=%x\n", mouse_qtd->token, mouse_qtd->buffer[0]);
    }
    
    // printf("Token=%x Active=%d Halted=%d\n", mouse_qtd->token, 
    //        (mouse_qtd->token & (1 << 7)) ? 1 : 0,
    //        (mouse_qtd->token & (1 << 6)) ? 1 : 0);
    ...
    if (!(mouse_qtd->token & (1 << 7))) { // If not active
        ...
        // Diagnostic for active token
        // printf("Wait: Token=%x Status=%x\n", mouse_qtd->token, operation_regs->usb_status);
    }
}
...
//         mouse_qh->horizontal_link_pointer = 1; // Terminate for periodic
//         // Address 1, EP, EPS=High (2), Max Packet, DTC=1 (IMPORTANT for interrupt!)
//         mouse_qh->endpoint_characteristics = 1 | (endpoint << 8) | (2 << 12) | (max_packet_size << 16) | (1 << 14);
//         // Multiplier=0 (1 transaction/uframe), S-mask=0x01 (microframe 0), RL=3
//         mouse_qh->endpoint_capabilities = (0 << 30) | (0x01 << 0) | (3 << 28);
